...
import { ENCHANTED_BADGE_TAG, ENCHANTED_BADGE_TAG_NAME, ENCHANTED_FAB_TAG, ENCHANTED_FAB_TAG_NAME } from '../../../components/tags';
...
describe(`${ENCHANTED_FAB_TAG_NAME} - component test`, () => {
  ...
  it("should render icon slot with correct part", async () => {
    renderComponent(html`<${ENCHANTED_FAB_TAG} .icon=${html`<icon-ai-sparkle></icon-ai-sparkle>`}></${ENCHANTED_FAB_TAG}>`);
    const fab = await $(ENCHANTED_FAB_TAG_NAME);
