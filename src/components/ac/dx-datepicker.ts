...
import { DEFAULT_DATE_FORMAT, MIN_YEAR, MAX_YEAR_BUFFER, CHANGE_CALENDAR_LAYOUT_TIMEOUT, DEFAULT_CALENDAR_LOCALE } from '../../types/dx-datepicker';
import {
...
export class DxDatePicker extends DxAcBaseElement {
  ...
  connectedCallback(): void {
    ...
    this._datePickerId = `dx-datepicker-${uuid()}`;
  ...
  _renderYearSelection() {
    setTimeout(() => {
      const yearSelectionDiv = this.renderRoot.querySelector(`#dx-datepicker-year-selection-${this._datePickerId}`);
      if (yearSelectionDiv) {
    ...
    return html`
      <div id="dx-datepicker-year-selection-${this._datePickerId}" part="${DATEPICKER_PARTS.DATEPICKER_YEAR_SELECTION}" style="visibility: hidden;">
